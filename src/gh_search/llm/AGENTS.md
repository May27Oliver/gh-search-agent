...
2. [`factory.py`](./factory.py) — 唯一的 provider 路由入口：
   ...
   - `canonical_model_name(...)` / `provider_for(...)` 把 alias（例：`claude-sonnet-4.5`）轉成 canonical name（`claude-sonnet-4`）。
   - `PROVIDER_BY_MODEL` / `MODEL_ALIASES` 是擴充 provider / model 時要改的地方。
...
4. [`prompts.py`](./prompts.py) — `load_prompt_bundle(name, model)` 讀 `prompts/core/{name}-v1.md` + 可選的 `prompts/appendix/{name}-{model}-v1.md`，回傳 `PromptBundle(core_text, appendix_text, prompt_version)`。PHASE2_PLAN §1.1 要求 model-specific 內容只能寫在 appendix，core 保持跨 provider 共用。
...
- `response_schema` 由 tool 自己產生，通常是 `SomePydanticModel.model_json_schema()`。llm/ 這層不知道任何 domain 細節。
- **沒有內建 retry**。provider 回錯直接 raise，由上層 tool / loop 用 `TerminateReason` 處理。

## 想換 model 或換 provider 的話

1. 在 `factory.py` 的 `MODEL_ALIASES` / `PROVIDER_BY_MODEL` 補 entry。
